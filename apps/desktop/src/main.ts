/** Wiring: mount the shell, route between the five screens, and keep the store fed from the
...
const TABS: Tab[] = ["library", "storage", "settings"];
...
async function start(): Promise<void> {
  await Promise.all([loadStatus(), loadSettings(), loadBootstrap(), loadClips(), loadProgress()]);
  renderToolbar();
  renderBanners();
  syncFirstRun();
  mount(current());

...
- The player reads clip files through the asset protocol, whose scope `lib.rs` widens to the clip folder and everything under it at startup and on every folder change. A video outside that folder will not play, silently. Builds from before per-game folders allowed only the top of the folder (and `Matches`), so a clip saved into `<Game>\Clips` does not play in one.
- **Per-game folders (2026-09-14):** a hotkey clip is moved after its probe from the top of the clip folder into `<clip folder>\<Game>\Clips` (`Unknown game\Clips` when undetected), and new session recordings go to `<clip folder>\<Game>\Matches`; `folders.rs` owns the naming. Nothing older was migrated: rows hold absolute paths, so loose clips at the top and match files in `<clip folder>\Matches` stay valid. Rename/merge and changing a clip's game move only clips already inside the old name's `Clips` folder (`storage::relocate`, all files or none, then empty folders are pruned). A clip whose file is open without `FILE_SHARE_DELETE` (ffmpeg) keeps its folder and logs a warning.
- Game pictures come from `game_art.rs` and are cached in `%APPDATA%\Cos Nostra\game-art\` (`index.json`, the slim Discord list `detectable.json`, the images); delete the folder to start over. Lookups run on one worker thread: Steam app id from the exe path (`libraryfolders.vdf` + `appmanifest_*.acf`) or from Discord's undocumented `GET /api/v9/applications/detectable` list (~13 MB on first use, refreshed weekly), then Steam's local `appcache\librarycache`, Steam CDN / store assets API, Discord's app icon and cover, and the exe's own icon. A lookup that finds nothing sends no `game-art-changed` event and is not retried for 24 h. A picture chosen in the game header is `custom` and never replaced automatically.
...
- Clip metadata lives in `%APPDATA%\Cos Nostra\clips.db` (SQLite, WAL). Delete it together with the `*.av1.mp4`, `*.h264.mp4` and `*.jpg` files next to the clips (at the top of the clip folder and in each `<Game>\Clips`) to start over.
- Game sessions and matches live in `sessions.db` beside it, their files in `<clip folder>\<Game>\Matches` (sessions from before per-game folders in `<clip folder>\Matches`). Delete both to start over; a session still marked `recording` at startup is ended and cut, so a killed app does not lose one.
